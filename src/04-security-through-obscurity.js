const input = ['vxupkizork-sgmtkzoi-pkrrehkgt-zxgototm-644[kotgr]',
...
const room = (name, sectorId, checksum) => ({ name, sectorId, checksum })

const parseRoomLabel = (roomLabel) => {
  const labelParts = /^(.+)-(\d+)\[([a-z]+)\]$/.exec(roomLabel)
  return room(labelParts[1], parseInt(labelParts[2]), labelParts[3])
}

const countCharacters = (roomName) => {

  const characterCounters = []

  roomName.split('').forEach((character) => {
    if (character === '-') {
      return
    }
    let characterCounter = characterCounters.filter((counter) => counter.character === character)[0]

    if (characterCounter) {
      characterCounter['count'] += 1
    } else {
      characterCounters.push({ character, count: 1 })
    }
  })
...
const createBuckets = (characterCounters) => {
  const buckets = []

  characterCounters.forEach((counter) => {
    let bucket = buckets.filter((b) => b['count'] === counter.count)[0]

    if (bucket) {
      bucket.characters.push(counter.character)
    } else {
      buckets.push({ 'count': counter.count, characters: [counter.character] })
    }
  })
...
const calculateChecksum = (roomName) => {
  ...
  const characterCounters = countCharacters(roomName)
  const buckets = createBuckets(characterCounters)
  sortBuckets(buckets)
  ...
  buckets.forEach((bucket) => {
    while (checksum.length < 5) {
      checksum.push(bucket.characters.shift())
      if (bucket.characters.length === 0) {
        break
      }
    }
  })
...
console.log(input.map((inputLine) => parseRoomLabel(inputLine)).map((room) => {
